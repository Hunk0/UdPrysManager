...
if(isset($_POST["registrar"])){
    ...
    if(isset($_POST["codigo"])){
        $codigo= $_POST["codigo"];
        $estudiante = new Estudiante ("", $nombre, $codigo, $mail, $pass);
            //esto no es necesario xdddd       
    }

    $paciente = new Paciente("", $nombre, $apellido, $correo, $clave, $cedula, 0);
    if(!$paciente -> existeCorreo()){
        $paciente -> registrar();
        $error = 0;
    }else{
        $error = 1;
